...
class MainPage(base_page.BasePage):
    ...
    def vk_id_visible(self):
        return self.visible(self.locators.VK_ID)

    def click_logout(self):
        self.click(self.locators.LOGOUT_BUTTON)

    def api_link_valid(self):
        link = self.get_value(locator=self.locators.API_CIRCLE, attribute_name="href")
        if link == "https://en.wikipedia.org/wiki/API":
            return True
        return f"Should be: 'https://en.wikipedia.org/wiki/API', got: {link}"

    def future_link_valid(self):
        link = self.get_value(locator=self.locators.FUTURE_INTERNET_CIRCLE, attribute_name="href")
        if link == "https://www.popularmechanics.com/technology/infrastructure/a29666802/future-of-the-internet/":
            return True
        return False

    def smtp_link_valid(self):
        link = self.get_value(locator=self.locators.SMTP_CIRCLE, attribute_name="href")
        if link == "https://ru.wikipedia.org/wiki/SMTP":
            return True
        return False

    def news_link_valid(self):
        link = self.get_value(locator=self.locators.NEWS_BUTTON, attribute_name="href")
        if link == "https://www.wireshark.org/news/":
            return True
        return False

    def download_link_valid(self):
        link = self.get_value(locator=self.locators.DOWNLOAD_BUTTON, attribute_name="href")
        if link == "https://www.wireshark.org/#download":
            return True
        return f"Should be: 'https://www.wireshark.org/#download', got: {link}"

    def examples_link_valid(self):
        link = self.get_value(locator=self.locators.EXAMPLES_BUTTON, attribute_name="href")
        if link == "https://hackertarget.com/tcpdump-examples/":
            return True
        return False

    def download_centos_link_valid(self):
        link = self.get_value(locator=self.locators.DOWNLOAD_CENTOS_BUTTON, attribute_name="href")
        if link == "https://www.centos.org/download/":
            return True
        return f"Should be: 'https://www.centos.org/download/', got: {link}"

    def python_link_valid(self):
        link = self.get_value(locator=self.locators.PYTHON_BUTTON, attribute_name="href")
        if link == "https://www.python.org/":
            return True
        return False

    def python_history_link_valid(self):
        link = self.get_value(locator=self.locators.PYTHON_HISTORY_BUTTON, attribute_name="href")
        if link == "https://en.wikipedia.org/wiki/History_of_Python":
            return True
        return False

    def flask_link_valid(self):
        link = self.get_value(locator=self.locators.ABOUT_FLASK_BUTTON, attribute_name="href")
